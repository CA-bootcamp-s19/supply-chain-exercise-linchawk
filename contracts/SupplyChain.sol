...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier sold (Item memory _item) {
    require(_item.state == State.Sold && _item.buyer != address(0));
    _;
  }

  modifier shipped (Item memory _item) {
    require(_item.state == State.Shipped && _item.buyer != address(0) && _item.seller != address(0));
    _;
  }

  modifier received (Item memory _item) {
    require(_item.state == State.Received && _item.buyer != address(0) && _item.seller != address(0));
    _;
  }
  ...
  function shipItem(uint sku) public sold(items[sku]) verifyCaller(items[sku].seller) {
  ...
  function receiveItem(uint sku) public shipped(items[sku]) verifyCaller(items[sku].buyer) {
